...
(function () {
  ...
  var S = '<svg xmlns="http://www.w3.org/2000/svg" style="display:none" aria-hidden="true">' +
    ...
    sym('chevron-down', '<path d="m6 9 6 6 6-6"/>') +
  ...
  function translateAttr(attr, dataAttr, lang) {
  ...
  function setLang(lang, persist) {
    ...
    translateAttr('placeholder', 'data-i18n-ph', lang);
    translateAttr('aria-label', 'data-i18n-aria', lang);
    translateAttr('content', 'data-i18n-content', lang);
    translateAttr('alt', 'data-i18n-alt', lang);
    var lt = document.getElementById('langToggle');
    if (lt) {
      var sp = lt.querySelector('span');
      if (sp) sp.textContent = lang === 'ar' ? 'English' : 'عربي';
  ...
  function updateWaLinks(lang) {
    var msg = lang === 'ar'
      ? 'مرحباً، أحتاج إلى خدمة سباكة في الدوحة. هل يمكنكم مساعدتي؟'
      : 'Hello, I need a plumbing service in Doha. Can you help?';
  ...
  /* Dropdown nav (desktop) — click toggles on touch devices */
  ...
  Array.prototype.forEach.call(navItems, function (item) {
    ...
    caret.addEventListener('click', function (e) {
      ...
      Array.prototype.forEach.call(navItems, function (other) {
        if (other !== item) {
  ...
  (function markActive() {
    ...
    var links = document.querySelectorAll('.nav a, .drawer .panel a, .nav .dropdown a, .drawer-sub a');
    for (var i = 0; i < links.length; i++) {
      var href = (links[i].getAttribute('href') || '').split('#')[0];
      try { href = decodeURIComponent(href); } catch (e) { }
      href = href.toLowerCase();
      if (href && (href === file || (file === 'index.html' && (href === '' || href === './' || href === 'index.html')))) {
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      e.preventDefault();
      var ar = isAr();
      var valOf = function (n) { var f = form.elements[n]; return f ? (f.value || '').trim() : ''; };
      var labelOf = function (n) {
        var f = form.elements[n];
        if (!f) return '';
        if (f.selectedOptions && f.selectedOptions[0]) return (f.selectedOptions[0].textContent || '').trim();
        return (f.value || '').trim();
      };
      var area = labelOf('area'), unit = labelOf('unit'), time = labelOf('time'), service = labelOf('service');
      var message = valOf('message');
      var lines = ar
        ? ['مرحباً، أود طلب خدمة.', 'المنطقة: ' + area, 'نوع الوحدة: ' + unit, 'الوقت المفضل للتنفيذ: ' + time, 'الخدمة المطلوبة: ' + service, message ? 'التفاصيل: ' + message : '']
        : ['Hello, I would like to request a service.', 'Area: ' + area, 'Unit type: ' + unit, 'Preferred time: ' + time, 'Service needed: ' + service, message ? 'Details: ' + message : ''];
